...
class run_proteinmpnn():
    ...
    def __peptide_design(self):
        ...
        # Optionally, in addition to sampling, compute the unconditional per-position
        # amino-acid profile of the peptide (the AA distribution that fits the same geometry).
        if self.mpnn_unconditional:
            self.__peptide_unconditional(output_dir, path_for_parsed_chains, path_for_assigned_chains)
        print('Full MHC Sequence Generation Mode Done! *****\n')

    def __peptide_unconditional(self, output_dir, path_for_parsed_chains, path_for_assigned_chains):
        """Run ProteinMPNN in unconditional mode for the peptide chain, alongside the sampling output.

        Reuses the parsed/assigned chain files already produced by __peptide_design. ProteinMPNN writes
        the raw per-position log-probabilities to <peptide_design>/unconditional_probs_only/<name>.npz
        (log_p of shape [B, L, 21], plus S/mask/design_mask). A convenience peptide profile CSV is also
        derived for the designed (peptide) positions.
        """
        print(f'***** Peptide Unconditional Profile Mode Start\n saving at {os.path.join(output_dir, "unconditional_probs_only")}')
        subprocess.run([
            "python", "-W", "ignore", "ProteinMPNN/protein_mpnn_run.py",
            "--jsonl_path", path_for_parsed_chains,
            "--chain_id_jsonl", path_for_assigned_chains,
        ...
        # derive a human-readable peptide amino-acid profile from the raw npz
        uncond_dir = os.path.join(output_dir, 'unconditional_probs_only')
        if os.path.isdir(uncond_dir):
            for npz in sorted(glob.glob(os.path.join(uncond_dir, '*.npz'))):
                try:
                    processing_functions.summarize_unconditional_peptide_profile(
                        npz, npz.replace('.npz', '_peptide_profile.csv'))
                except Exception as e:
                    print(f'## Warning: could not summarize unconditional profile {npz}: {e} ##')
        print('Peptide Unconditional Profile Mode Done! *****\n')
