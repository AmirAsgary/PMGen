...
class run_proteinmpnn():
    def __init__(self, PMGen_pdb, output_dir,
                ...
                peptide_chain='P', mhc_design=True, peptide_design=True,
                 ...
                 model_name='v_48_020', only_multichain=True, mpnn_unconditional=False,
                 mpnn_conditional=False):
        '''
        Args:
        ...
        self.mpnn_unconditional = mpnn_unconditional
        self.mpnn_conditional = mpnn_conditional
        self.input_assertion()
    ...
    def __peptide_design(self):
        ...
        # Optionally, in addition to sampling, compute the per-position amino-acid profile of the peptide:
        #  - unconditional: the AA distribution that fits the backbone geometry alone.
        #  - conditional:   the AA distribution at each position given the rest of the sequence + backbone.
        if self.mpnn_unconditional:
            self.__peptide_probs('unconditional', output_dir, path_for_parsed_chains, path_for_assigned_chains)
        if self.mpnn_conditional:
            self.__peptide_probs('conditional', output_dir, path_for_parsed_chains, path_for_assigned_chains)
        print('Full MHC Sequence Generation Mode Done! *****\n')

    def __peptide_probs(self, mode, output_dir, path_for_parsed_chains, path_for_assigned_chains):
        """Run ProteinMPNN in (un)conditional-probability mode for the peptide chain, alongside sampling.

        mode: 'unconditional' or 'conditional'. Reuses the parsed/assigned chain files already produced by
        __peptide_design. ProteinMPNN writes the raw per-position log-probabilities to
        <peptide_design>/<mode>_probs_only/<name>.npz (log_p [B, L, 21] plus S/mask/design_mask). A
        convenience peptide profile CSV + sequence-logo SVG are derived for the designed (peptide) positions.
        """
        assert mode in ('unconditional', 'conditional'), f'unknown probs mode {mode}'
        probs_dir = os.path.join(output_dir, f'{mode}_probs_only')
        print(f'***** Peptide {mode.capitalize()} Profile Mode Start\n saving at {probs_dir}')
        # The profile is computed in a single batch; protein_mpnn_run uses NUM_BATCHES = num_seq_per_target //
        # batch_size, so we force both to 1 to guarantee NUM_BATCHES >= 1 regardless of the sampling batch_size.
        subprocess.run([
            "python", "-W", "ignore", "ProteinMPNN/protein_mpnn_run.py",
            "--jsonl_path", path_for_parsed_chains,
            "--chain_id_jsonl", path_for_assigned_chains,
            "--out_folder", output_dir,
            f"--{mode}_probs_only", "1",
        ...
        # derive a human-readable peptide amino-acid profile (+ logo) from the raw npz
        if os.path.isdir(probs_dir):
            for npz in sorted(glob.glob(os.path.join(probs_dir, '*.npz'))):
                try:
                    processing_functions.summarize_mpnn_peptide_profile(
                        npz, npz.replace('.npz', '_peptide_profile.csv'), label=mode)
                except Exception as e:
                    print(f'## Warning: could not summarize {mode} profile {npz}: {e} ##')
        print(f'Peptide {mode.capitalize()} Profile Mode Done! *****\n')
...
def run_single_proteinmpnn(path, directory, args, anchor_and_peptide=None):
    ...
    runner_mpnn = run_proteinmpnn(
        ...
        mpnn_unconditional=getattr(args, 'mpnn_unconditional', False),
        mpnn_conditional=getattr(args, 'mpnn_conditional', False),
    )
    runner_mpnn.run() #
...
def save_peptide_plddt_results(output_dir, df):
    """Write per-structure peptide mean pLDDT to <output_dir>/results.csv after AlphaFold finishes."""
    return processing_functions.compute_peptide_plddt_results(df=df, output_folder=output_dir,
                                                              path_to_af='alphafold')
    ...
    for id in id_paths:
        ...
        if os.path.isdir(idp):
            ...
            if len(model_folders) > 0:
                try:
                    # pick the model sub-folder that actually holds peptide_design
                    # (works whether there is one or several model folders)
                    model_folder = next(
